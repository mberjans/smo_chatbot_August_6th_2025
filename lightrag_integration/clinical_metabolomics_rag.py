...
class CostSummary:
    """
    Summary of API costs and usage statistics for Clinical Metabolomics RAG operations.
    
    This data class provides a comprehensive overview of the financial and usage metrics
    for OpenAI API calls made during RAG operations. It tracks token usage, costs, and
    query statistics to help monitor budget and optimize system usage.
    
    Attributes:
        total_cost: Total monetary cost in USD for all API operations
        total_queries: Total number of queries processed by the system
        total_tokens: Total number of tokens used across all operations
        prompt_tokens: Number of tokens used in prompts (input)
        completion_tokens: Number of tokens generated in completions (output)
        embedding_tokens: Number of tokens used for text embeddings
        average_cost_per_query: Mean cost per query in USD
        query_history_count: Number of queries stored in history
        
    Example:
        ```python
        cost_summary = rag.get_cost_summary()
        print(f"Total cost: ${cost_summary.total_cost:.3f}")
        print(f"Average per query: ${cost_summary.average_cost_per_query:.4f}")
        print(f"Efficiency: {cost_summary.total_tokens / cost_summary.total_queries:.1f} tokens/query")
        ```
    """
    total_cost: float
    total_queries: int
...
class QueryResponse:
    """
    Response from a RAG query operation with detailed metadata and cost tracking.
    
    This data class encapsulates the complete response from a Clinical Metabolomics RAG
    query, including the generated content, processing metadata, cost information, and
    performance metrics. It provides structured access to all aspects of the query result.
    
    Attributes:
        content: The main response content generated by the RAG system
        metadata: Dictionary containing query metadata including:
            - sources: List of source documents used
            - query_id: Unique identifier for the query
            - timestamp: When the query was processed
            - lightrag_metadata: Internal LightRAG metadata
            - extracted_entities: Biomedical entities found in response
            - formatted_response: Structured response data (if available)
        cost: Estimated API cost for this query in USD
        token_usage: Dictionary with token consumption details:
            - prompt_tokens: Input tokens used
            - completion_tokens: Output tokens generated
            - total_tokens: Combined token count
        query_mode: Query processing mode used ('naive', 'local', 'global', 'hybrid')
        processing_time: Total processing time in seconds
        
    Example:
        ```python
        response = await rag.query("What are the metabolites in glucose metabolism?")
        print(f"Content: {response.content[:100]}...")
        print(f"Cost: ${response.cost:.4f}")
        print(f"Sources: {len(response.metadata.get('sources', []))}")
...
class ClinicalMetabolomicsRAGError(Exception):
    """
    Base exception class for all Clinical Metabolomics RAG system errors.
    
    This is the root exception class for all errors that can occur within the
    Clinical Metabolomics RAG system. It provides a common base for error handling
    and allows for broad exception catching when needed.
    
    All specific error types (QueryError, IngestionError, etc.) inherit from this
    base class, enabling hierarchical error handling strategies.
